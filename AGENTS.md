...
- 股市项目首期严格只做一个公开、连续滚动的一级首页；顶部导航使用页内锚点，不创建或伪造登录、定价、社区、专栏、文档、个股详情等二级入口。
- 一级首页先补齐市场与股票、K 线与成交量、公司基本面、估值与回报、交易规则与风险、研究与复盘六组最小知识，再展示“观察—提问—验证—复盘”研究闭环。
- 页面和示例必须保持教育属性，不提供荐股、选股排名、目标价、仓位、买卖时点、代客交易或收益承诺；AI 只辅助整理信息与检查证据，重要事实回到官方或原始来源核验。
- 规则型内容必须来自监管机构或交易所现行文件并显示最近核对日期；历史、延时或教学行情必须标明数据来源、日期和延迟，未接入真实实时数据时禁止显示 LIVE。
- `docs/stockchart/LEVEL_ONE_PRD_V1.md`、`FUNCTION_MAP_V1.md` 与 `STOCK_BASICS_V1.md` 是当前股市项目一级界面的规划基线；进入视觉设计或实现前先确认最终品牌、数据授权与教学图表示例。
