...
const trustItems = [
  {
    ...
    zh: '推演依据',
    en: 'Reading basis',
    descZh: '盘面结构与判断逻辑分开展示，不混为一谈',
    descEn: 'Chart structure and interpretation are shown separately.',
  },
  {
    glyph: '卷',
    zh: '古籍出处',
    en: 'Classic sources',
    descZh: '解读锚定古籍摘句，保留来源与上下文',
    descEn: 'Interpretations retain their source and context.',
  },
  {
    glyph: '图',
    zh: '知识图解',
    en: 'Knowledge maps',
    descZh: '结构图随结果出现，看懂概念之间的关系',
    descEn: 'Diagrams reveal the relationships behind the result.',
  },
  {
    glyph: '词',
    zh: '术语解释',
    en: 'Term notes',
    descZh: '生僻术语就地展开，不用离开当前页面',
    descEn: 'Unfamiliar terms open in context without leaving the page.',
  },
]
...
export function HomePage() {
  ...
  return (
    <div className="fate-home" ref={homeRef}>
      ...
      <section className="fate-trust fate-motion-section">
        <header className="fate-centered-head">
          <span className="fate-section-tag" data-reveal>WHY TRUST · {t('结果从哪来', 'WHERE RESULTS COME FROM')}</span>
          <h2 data-reveal>{t('每句解读，都有出处', 'Every reading has a source')}</h2>
        </header>
        <div className="fate-trust-grid">
          {trustItems.map((item) => (
            <div className="fate-trust-item" key={item.zh} data-reveal>
              <span>{item.glyph}</span>
              <h3>{lang === 'en' ? item.en : item.zh}</h3>
              <p>{lang === 'en' ? item.descEn : item.descZh}</p>
            </div>
          ))}
        </div>
      </section>
